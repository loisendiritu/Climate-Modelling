...
The goal of this project is to numerically simulate how the planetary temperature of a naked planet would change through time as it approaches equilibrium i.e. the state at which it stops changing. The planet starts with some initial temperature. "Naked" in means that we are not accounting for the atmospheric layers that cause the greenhouse effect.
